...
fetchDocuments = async () => {
    ...
    // group items
    var items = Object.keys(documentMapping).reduce(
        (acc, documentationId) =>
            acc.concat(
...
fetchDocuments_single = async documentationId => {
    ...
    let result = await (
        await fetch(`https://developer.salesforce.com/docs/get_document/${documentationId}`)
    ).json();
    //console.log(documentationId,result);
    var items = removeDuplicates(extraDataFromJson(documentationId, result.toc, []), 'id').sort(
...
fetchContentDocument = async (documentationId, url) => {
    try {
        ...
        const res = await (
            await fetch(
                `https://developer.salesforce.com/docs/get_document_content/${header.deliverable}/${url}/en-us/${header.version.doc_version}`
            )
        ).json();
...
loadAllDocuments = async items => {
    // Chunk the URLs
    const itemChunks = chunkList(
        items.map(x => ({ url: x.a_attr.href, ...x })),
        CHUNK_SIZE
    );
    let finalResult = [];
    // Process each chunk
