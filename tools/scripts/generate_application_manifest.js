...
// Slash-command metadata an app can contribute. Schema validated below —
// the values are surfaced verbatim by the agent publisher's autocomplete.
const SLASH_COMMAND_PATTERN = /^[a-z][a-z0-9_-]*$/;
const SLASH_COMMAND_MAX_LEN = 32;
const ALLOWED_SLASH_COMMAND_KEYS = new Set([
    'command',
    'description',
    'iconName',
    'autoExecute',
...
const ALLOWED_MENU_GROUPS = new Set(['data', 'code', 'agentforce', 'admin', 'utilities']);
...
function validateSlashCommands(manifest, errors) {
    if (!('slashCommands' in manifest)) return;
    const list = manifest.slashCommands;
    ...
    list.forEach((entry, idx) => {
        const prefix = `slashCommands[${idx}]`;
        if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
            errors.push(`${prefix} must be an object`);
            return;
        }
        for (const key of Object.keys(entry)) {
            if (!ALLOWED_SLASH_COMMAND_KEYS.has(key)) {
                errors.push(`${prefix} has unknown key "${key}"`);
            }
        }
        const { command, description, iconName, autoExecute, commandId } = entry;
        if (typeof command !== 'string' || !SLASH_COMMAND_PATTERN.test(command)) {
            errors.push(`${prefix}.command must match ${SLASH_COMMAND_PATTERN}`);
        } else if (command.length > SLASH_COMMAND_MAX_LEN) {
            errors.push(`${prefix}.command must be ${SLASH_COMMAND_MAX_LEN} chars or fewer`);
        } else if (seen.has(command)) {
            errors.push(`${prefix}.command "${command}" duplicated in the same manifest`);
        } else {
            seen.add(command);
        }
        ...
        if (typeof iconName !== 'string' || !ICON_PATTERN.test(iconName)) {
            errors.push(`${prefix}.iconName "${iconName}" must match ${ICON_PATTERN}`);
        }
        if (autoExecute !== undefined && typeof autoExecute !== 'boolean') {
            errors.push(`${prefix}.autoExecute must be boolean`);
        }
        if (typeof commandId !== 'string' || commandId.length === 0) {
            ...
            !commandId.startsWith(`${manifest.id}.`)
        ) {
            errors.push(
                `${prefix}.commandId "${commandId}" must start with "${manifest.id}." (own-your-namespace)`
            );
        }
    });
}
...
function validate(manifest, filePath) {
    ...
    validateSlashCommands(manifest, errors);
...
function renderGeneratedRegistryFlattened(manifests) {
    ...
    const slashCommandLines = [];
    for (const m of sorted) {
        if (!Array.isArray(m.slashCommands) || m.slashCommands.length === 0) continue;
        for (const sc of m.slashCommands) {
            const fields = [
                `command: ${jsStringLiteral(sc.command)}`,
                `description: ${jsStringLiteral(sc.description)}`,
                `iconName: ${jsStringLiteral(sc.iconName)}`,
                `autoExecute: ${sc.autoExecute !== false ? 'true' : 'false'}`,
                `commandId: ${jsStringLiteral(sc.commandId)}`,
                `appId: ${jsStringLiteral(m.id)}`,
            ];
            slashCommandLines.push(`    { ${fields.join(', ')} },`);
        }
    }
    const slashCommandsExport = `const APPLICATION_SLASH_COMMANDS = [\n${slashCommandLines.join('\n')}${slashCommandLines.length > 0 ? '\n' : ''}];\n`;
...
function generate() {
    ...
    const seenSlashCommands = new Map();
    for (const m of manifests) {
        if (Array.isArray(m.slashCommands)) {
            for (const sc of m.slashCommands) {
                if (seenSlashCommands.has(sc.command)) {
                    throw new Error(
                        `Duplicate slash command "/${sc.command}" in ${m.name} — also used by ${seenSlashCommands.get(sc.command)}`
                    );
                }
                seenSlashCommands.set(sc.command, m.name);
            }
        }
        if (seenIds.has(m.id)) {
            throw new Error(
