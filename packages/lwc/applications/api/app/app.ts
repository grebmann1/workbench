...
let _apiBootstrapped = false;
function bootstrapApiExtension() {
...
export default class App extends ToolkitElement {
    ...
    recentApiItems: Array<Record<string, any>> = [];
    savedApiItems: Array<Record<string, any>> = [];

    // API Tree
    @track isApiTreeToggled = false;
    @track apiTreeItems: Array<Record<string, any>> = [];

    // Salesforce Catalog
    @track isCatalogToggled = false;
    @track apiCatalogItems: Array<Record<string, any>> = [];
    @track isCatalogLoading = false;
    @track apiCatalogError: string | null = null;

    // Content
    @track content: string | null = null;
    @track contentLength: number | null = null;
    @track contentType = 'json';
    @track contentHeaders: Array<Record<string, any>> = [];
    ...
    storeChange({ application, api, recents, apiFiles, openapiSchemaFiles }) {
        ...
        // Api State
        const apiState = apiLocalSelectors.selectById({ api }, lowerCaseKey(api.currentTab?.id));
        if (apiState) {
