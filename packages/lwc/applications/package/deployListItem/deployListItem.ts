...
export default class Message extends ToolkitElement {
    ...
    @api selectedItemId: string | null = null;

    _formattedCreatedDate: string | null = null;
    _formattedDurationDate: string | null = null;
    ...
    formatDate = (): void => {
        ...
        if (isNotUndefinedOrNull(this.item.StartDate)) {
            if (isNotUndefinedOrNull(this.item.CompletedDate)) {
                this._formattedDurationDate = moment
                    .duration(moment(this.item.CompletedDate).diff(moment(this.item.StartDate)))
                    .humanize();
            } else {
                this._formattedDurationDate = moment
                    .duration(moment().diff(moment(this.item.StartDate)))
                    .humanize();
            }
        }
    };
    ...
    get isFailure() {
        return (
            ...
            this.item.Status === 'SucceededPartial'
        );
    }
    ...
    get formattedTests() {
        ...
        const percentage =
            this.item.NumberTestsTotal > 0
                ? ' ( ' + this.calculatePercentage(total, this.item.NumberTestsTotal) + '% )'
                : null;
