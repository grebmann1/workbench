...
export default class Deploy extends ToolkitElement {
    ...
    // requests
    @track requests: AnyRecord[] = [];

    _selectedItem: AnyRecord | null = null;
    get selectedItem(): AnyRecord | null {
        return this._selectedItem;
    }

    set selectedItem(value: AnyRecord | null) {
    ...
    // Viewer
    viewer_value: string = API_UTILS.VIEWERS.PRETTY;
    ...
    cleanRestResponse = (obj: AnyRecord | AnyRecord[]): AnyRecord | AnyRecord[] => {
    ...
    formatObjectToJson = (obj: AnyRecord | null): string => {
        return JSON.stringify(obj, null, 4);
    };

    /** Events **/

    handleAbort = async (e: any): Promise<void> => {
        //CanceledBy
        try {
            const cancelResult = await this.connector.conn.metadata._invoke('cancelDeploy', {
    ...
    viewer_handleChange = (e: any): void => {
    ...
    get abortLabel() {
        return isUndefinedOrNull(this.selectedItem.CanceledById)
            ? this.i18n.EDITOR_PANEL_ABORT
            : this.i18n.EDITOR_PANEL_ABORTING;
    }

    get isAbortDisabled() {
        return (
            !['InProgress', 'Pending'].includes(this.selectedItem?.Status) ||
            isNotUndefinedOrNull(this.selectedItem?.CanceledById)
        );
    }
}
