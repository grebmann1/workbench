...
export default class Retrieve extends ToolkitElement {
    ...
    @api
    toggleMetadata = async (param: AnyRecord): Promise<void> => {
    ...
    updateManifest = async (
        xml: string,
        membersToToggle: Record<string, string[]>,
        selectAll: string[],
        unselectAll: string[]
    ): Promise<string> => {
        try {
            ...
            const createTypesNode = (typeName: string) => {
            ...
            const setMembers = (entry: AnyRecord, members: string[]) => {
    ...
    loading_formatDate = (createdDate: string | number | Date): void => {
    ...
    loading_enableAutoDate = (createdDate: string | number | Date): void => {
        if (this._loadingInterval) clearInterval(this._loadingInterval);
    ...
    handle_downloadClick = (e: any): void => {
