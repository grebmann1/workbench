...
const getAccessAnalyzerJobMeta = (alias, startedAt = Date.now()) => {
    const resolvedAlias = alias || 'unknown';
    return {
        id: `access-analyzer-${resolvedAlias}-${startedAt}`,
        category: 'accessAnalyzer',
        label: `Access Analyzer (${resolvedAlias})`,
        source: 'accessAnalyzer.worker',
        startedAt,
    };
};
...
export default class App extends ToolkitElement {
    ...
    filter_profiles: string[] = [];
    filter_permissionSets: string[] = [];
    filter_users: string[] = [];
    ...
    /* Matrix */
    greenTreshold = 5;
    _greenTreshold: number | null = null;
    orangeTreshold = 10;
    _orangeTreshold: number | null = null;

    isLoading = false;
    customLoadingMessage: string | null = null;
    customAdditionalMessage: string | null = null;

    dataList: AnyRecord[] = [];
    _tableInstance: any = null;
    ...
    loadFromWebWorker = async () => {
        return new Promise((resolve, reject) => {
            ...
            const jobMeta = getAccessAnalyzerJobMeta(this.connector?.conn?.alias, startedAt);
            store.dispatch(
                BACKGROUNDJOB.reduxSlice.actions.upsertJob({
                    ...jobMeta,
                    status: 'running',
                    phase: 'init',
                    message: 'Starting Access Analyzer worker',
                    startedAt,
                    updatedAt: Date.now(),
                })
            );
                ...
                if (status === 'running') {
                    if (type === 'callback') {
                        ...
                        store.dispatch(
                            BACKGROUNDJOB.reduxSlice.actions.upsertJob({
                                ...
                                status: 'running',
                                phase: 'namespace',
                                message: 'Loading namespace metadata',
                                updatedAt: Date.now(),
                            })
                        );
                    ...
                    store.dispatch(
                        BACKGROUNDJOB.reduxSlice.actions.completeJob({
                            ...jobMeta,
                            phase: 'done',
                            message: 'Access Analyzer job finished',
                            resultSummary: 'Metadata loaded for report generation',
                            updatedAt: Date.now(),
                        })
                    );
                    resolve(value);
                    ...
                    store.dispatch(
                        BACKGROUNDJOB.reduxSlice.actions.failJob({
                            ...jobMeta,
                            phase: 'error',
                            message: 'Access Analyzer job failed',
                            error: value,
                            updatedAt: Date.now(),
                        })
                    );
