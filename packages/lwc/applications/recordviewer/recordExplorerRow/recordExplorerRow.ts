...
export default class RecordExplorerRow extends ToolkitElement {
    ...
    hasRendered = false;
    // Editing
    @api editingFieldSet: Array<Record<string, any>> | null = null; // used for scroll loading !
    @api isEditMode = false;
    @api isDirty = false;
    @api record: Record<string, any> | null = null;
    @api metadata: Record<string, any> | null = null;

    _filter: string | null = null;
    _isLabelDisplayed: boolean | null = null;
    _errors: Record<string, any> | null = null;

    @api get filter(): string | null {
        return this._filter;
    }
    ...
    // Field & Label
    @api get isLabelDisplayed(): boolean | null {
        return this._isLabelDisplayed;
    }

    set isLabelDisplayed(value: boolean | null) {
    ...
    @api get isInfoDisplayed(): boolean {
        return this._isInfoDisplayed;
    }

    set isInfoDisplayed(value: boolean) {
    ...
    @api get fieldErrors(): Record<string, any> | null {
        return this._errors;
    }

    set fieldErrors(value: Record<string, any> | null) {
        this._errors = value;
    ...
    get isPicklist() {
        return (
            this.item.fieldInfo.type == 'picklist' || this.item.fieldInfo.type == 'multipicklist'
        );
    }
        ...
        if (
            ...
            ((isNotUndefinedOrNull(this.editingFieldSet) && this.editingFieldSet.has(this.name)) || // Editing Mode
                (isNotUndefinedOrNull(this.fieldErrors) &&
                    this.fieldErrors.hasOwnProperty(this.name))) // Error Mode
        ) {
    ...
    renderFieldErrors = () => {
        window.setTimeout(() => {
            if (
                this.hasRendered /* && this.isEditMode */ &&
                isNotUndefinedOrNull(this.fieldErrors) &&
    ...
    fetchAllRecordTypes = async recordTypeId => {
        ...
        const fetchAllPicklists = async recordType => {
            const res = await this.connector.conn.request(
                `/ui-api/object-info/${this.metadata.name}/picklist-values/${recordType.recordTypeId}`
            );
            return {
    ...
    handleDependentPicklistClick = async () => {
        ...
        try {
            const matrix = await this.generateMatrix(
                this.item.fieldInfo.controllerName,
                this.item.fieldInfo.name
            );
