...
const SELECTORS = { platformEvents: platformEventSelectors };

let _platformEventBootstrapped = false;
function bootstrapPlatformEventExtension() {
    if (_platformEventBootstrapped) return;
    _platformEventBootstrapped = true;
...
export default class App extends ToolkitElement {
    ...
    @track recentChannels: Array<Record<string, any>> = [];
    @track eventObjects: Array<Record<string, any>> = [];
    @track pushTopics: Array<Record<string, any>> = [];

    // Used to store platformEvent records
    @track subscribedChannels: Array<Record<string, any>> = [];
    @track activeSubscriptions: Record<string, any> = {};
    currentChannel: Record<string, any> | null = null;
    currentChannelName: string | null = null;

    lastMessageFormatted = DEFAULT_LAST_MESSAGE;

    extensionInstance: any = null;

    /** */
    @track messagesDisplayed: Array<Record<string, any>> = []; //[{id:1712150227604,receivedDate:1712150227604,content:{channel:"/event/CCR_TaskNotification__e",data:{event:{EventApiName:"CCR_TaskNotification__e",EventUuid:"b2d74e15-07db-4856-81f2-aa65e904bb64",replayId:1499957}}}}];
    @track selectedEventItem: Record<string, any> | null = null; // = this.messagesDisplayed[0];

    /** Lookup */
    @track lookup_selectedEvents: Array<Record<string, any>> = [];
    ...
    handleEventSelection = (e: any): void => {
        e.stopPropagation();
        e.preventDefault();
