...
export default class Utilization extends ToolkitElement {
    userLicenses: Array<Record<string, any>> = [];
    ...
    getCurrentRank = (mapping: number[], value: number): number => {
    ...
    load_userLicenses = async (): Promise<Array<Record<string, any>>> => {
        const response = await this.connector.conn.query(
            "SELECT CreatedDate, Id, LastModifiedDate, LicenseDefinitionKey, MasterLabel, Name, Status, SystemModstamp, TotalLicenses, UsedLicenses, UsedLicensesLastUpdated FROM UserLicense WHERE Status = 'Active' ORDER BY UsedLicenses DESC"
        );
