...
export default class App extends LightningElement {
    ...
    getStructuredDataByFiles = () => {
        ...
        this.getFilteredData.forEach((x, index) => {
            ...
            files[path].items.push(new Item(guid(), level, ruleId, message.text, _location));
        });
    ...
    getStructuredData = () => {
        ...
        this.getFilteredData.forEach((x, index) => {
            ...
            if (!result[level].rules.hasOwnProperty(ruleId)) {
                ...
                result[level].rules[ruleId] = new Rule(ruleId, rule, result[level].label);
            }
            let _location = locations[0].physicalLocation;
            let path = _location.artifactLocation.uri;

            if (!result[level].rules[ruleId].files.hasOwnProperty(path)) {
                result[level].rules[ruleId].files[path] = new FileItem(guid(), path, level, true);
            }

            result[level].rules[ruleId].files[path].items.push(
                new Item(guid(), level, ruleId, message.text, _location)
            );
        });
    ...
    get getFilteredData() {
        ...
        return this.sarif_run.results.filter(x => {
            ...
            return (
                ...
                message.text.toLowerCase().includes(this.filter.toLowerCase())
            );
        });
    }
}
