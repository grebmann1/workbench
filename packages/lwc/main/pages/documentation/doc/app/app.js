...
export default class App extends ToolkitElement {
    ...
    loadFromNavigation = async ({ state }) => {
        ...
        if (attribute1) {
            ...
            if (
                !this.cloud_value.includes(attribute1) &&
                CONFIGURATION.hasOwnProperty(attribute1)
            ) {
    ...
    loadFromCache = async () => {
        ...
        this.cloud_value = getFromStorage(localStorage.getItem(DOCUMENTATION_CLOUD_VALUE), [
            DEFAULT_CONFIG,
        ]);
    };
    ...
    refreshDocumentsFromCloud = async () => {
        window.setTimeout(async () => {
            if (!isEmpty(this.filter)) {
                this.filteredItems = removeDuplicates(
                    await this.getFilteredItems(this.filter),
                    'id'
                );
            }
            this.fetchDocuments();
        ...
        if (!isEmpty(this.filter)) {
            ...
            if (regex.test(formattedContent)) {
                formattedContent = formattedContent
                    .toString()
                    .replace(regex, `<span style="font-weight:Bold; color:blue;">$2</span>`);
            }
        }
    ...
    transformCodeBlockToComponents = () => {
        ...
        this.refs.container.querySelectorAll('.codeSection').forEach(el => {
    ...
    extractData = () => {
        ...
        [...items].forEach(x => {
            ...
            x.querySelectorAll('.detailList .dt').forEach(dt => {
                ...
                data[(dt.textContent || '').replace(/ /g, '')] = (dd.textContent || '').replace(
                    / /g,
                    ''
                );
            });
    ...
    buildUML = async () => {
        ...
        const source = this.concatenateNonEmptyText(
            this.refs.container.querySelector('.helpHead1')
        ).replace(/ /g, '');
    ...
    fetchDocuments = async () => {
        ...
        this.items = Object.keys(this.documentMapping)
            ...
            .reduce(
                (acc, documentationId) =>
                    acc.concat(
                        this.documentMapping[documentationId].items.map(x => ({
                            ...x,
                            documentationId,
                        }))
                    ),
    ...
    fetchDocuments_single = async documentationId => {
        ...
        let result = await (
            await fetch(`${this.apiDomain}/docs/get_document/${documentationId}`)
        ).json();
    ...
    fetchContentDocument = async (deliverable, contentDocumentId, version) => {
        return await (
            await fetch(
                `${this.apiDomain}/docs/get_document_content/${deliverable}/${contentDocumentId}/${this.language}/${version}`
            )
        ).json();
    };
    ...
    cleanContent = content => {
        ...
        if (srcRegex.test(content)) {
            content = content
                .toString()
                .replace(srcRegex, 'src="https://developer.salesforce.com/');
        }
    ...
    get documentationTitle() {
        return `Documentation Explorer ${
            isNotUndefinedOrNull(this.currentSobject) ? ` - ${this.currentSobject.title}` : ''
        }`;
    }

    get searchHost() {
        return this.isResponsive ? 'https://www.sf-workbench.com/' : '/';
    }
    ...
    get menuContainerClass() {
        return classSet(
            'slds-flex-column slds-p-left_x-small slds-full-height slds-col min-height-200 background-gray'
        )
            .add({
    ...
    get documentationContainerClass() {
        return classSet(
            'documentation-container slds-scrollable_y slds-p-horizontal_small slds-flex-column slds-full-height slds-col slds-is-relative'
        )
            .add({
