...
export default class FileTreeItem extends LightningElement {
    ...
    handleClick(event) {
        // `lightning-formatted-rich-text` can auto-linkify text into `<a>` tags.
        ...
        if (this.isFolder) {
            if (this.enableToggleOnItemClick === true) {
    ...
    handleMenuSelect(event) {
        ...
        if (value === 'expandAll') {
            ...
            this.dispatchItemAction('delete');
        } else if (value) {
            this.dispatchItemAction(value);
        }
    }
    ...
    get isFolderIconDisplayedAndVisible() {
        return !this.hideIcons && this.isFolderIconDisplayed;
    }
    ...
    get customActions() {
        const actions = Array.isArray(this.item?.actions) ? this.item.actions : [];
        return actions
            .map(action => ({
                name: String(action?.name || '').trim(),
                label: String(action?.label || '').trim(),
                iconName: String(action?.iconName || '').trim(),
                disabled: action?.disabled === true,
            }))
            .filter(action => action.name && action.label);
    }
    ...
    get isFolderMenuVisible() {
        return this.isFolder && this.isMenuButtonVisible;
    }
