...
export default class Paginator extends LightningElement {
    ...
    setRecordsToDisplay() {
        ...
        this.totalPages =
            Math.ceil(this._records.length / this.pageSize) == 0
                ? 1
                : Math.ceil(this._records.length / this.pageSize);
    ...
    handleKeyChange(event) {
        ...
        if (searchKey) {
            this.delayTimeout = setTimeout(() => {
                ...
                this.recordsToDisplay = this.records.filter(
                    rec =>
                        isNotUndefinedOrNull(rec[this.filterField]) &&
                        rec[this.filterField].includes(searchKey)
                );
