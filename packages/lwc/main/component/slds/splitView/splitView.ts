...
export default class SplitView extends LightningElement {
    ...
    get leftColumnStyle() {
        return this.isHorizontal
            ? `width: ${this.splitterPosition}%;`
            : `height: ${this.splitterPosition}%;`;
    }

    get rightColumnStyle() {
        return this.isHorizontal
            ? `width: calc(${100 - this.splitterPosition}% - 12px);`
            : `height: calc(${100 - this.splitterPosition}% - 12px);`;
    }

    get splitterStyle() {
        return this.isHorizontal
            ? `left: ${this.splitterPosition}%;`
            : `top: ${this.splitterPosition}%;`;
    }
    ...
    get splitterClass() {
        return classSet('splitter')
            .add(this.separatorDirection)
            .add({
