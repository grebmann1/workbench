...
- `packages/lwc/main/core/applications/applications.ts`

Application-specific entries and menu groups are still declared in:
- `packages/lwc/main/component/skeleton/registry/registry.ts`
...
```text
packages/lwc/main/application/<appFolder>/
  app/
    app.js
    app.html
    app.css (optional)
...
1. Create your app component in this folder:
   - `packages/lwc/main/application/<appFolder>/app/`
2. In `packages/lwc/main/component/skeleton/registry/registry.ts`, import the module:
...
Current build section groups are declared in `APPLICATION_MENU_GROUPS` and exposed by `packages/lwc/main/core/applications/applications.ts`:
- `data`
- `code`
- `explorers`
- `deploy`
