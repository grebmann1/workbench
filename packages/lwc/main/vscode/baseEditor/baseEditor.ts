...
function getDefaultMonacoWorkerFactory() {
    return async logger => {
        ...
        defaultworkerLoaders.editorWorkerService = () => {
            return new Worker(EDITOR_WORKER_URL, { type: 'module' });
        };
        defaultworkerLoaders.TextMateWorker = () => {
            return new Worker(TEXTMATE_WORKER_URL, { type: 'module' });
        };
...
function buildDefaultVscodeApiConfig(userConfig, logLevel = LogLevel.Off) {
    ...
    if (userConfig) {
        return {
            ...
            monacoWorkerFactory:
                userConfig.monacoWorkerFactory ?? defaultConfig.monacoWorkerFactory,
...
export default class BaseEditor extends LightningElement {
    ...
    connectedCallback() {
        ...
        this._editorId =
            this._editorId || Math.floor(Math.random() * (MAX_EDITOR_ID + 1)).toString();
        this.asyncInit();
    }
    ...
    async initializeEditor() {
        ...
        try {
            ...
            if (
                updatedViewServiceType !== 'EditorService' &&
                updatedViewServiceType !== undefined
            ) {
    ...
    async buildOriginalModelRef() {
        const original = {
            ...
            uri:
                this.config?.codeResources?.original?.uri ??
                `default-uri-original-${this.editorId}`,
    ...
    async buildModelRefs() {
        const modified = {
            ...
            uri:
                this.config?.codeResources?.modified?.uri ??
                `default-uri-modified-${this.editorId}`,
    ...
    async updateDiffEditorModel() {
        ...
        if (modified !== undefined && original !== undefined) {
            ...
            this.logger.warn(
                'Diff Editor: Both modified and original model references are required'
            );
        }
    }
    ...
    async setModelRefs(modelRefs) {
        ...
        if (this.useDiffEditor) {
            if (updateModified || updateOriginal) {
                ...
                this.logger.info(
                    'Diff Editor: Model references were not updated. They are either unchanged or undefined.'
                );
            }
            ...
            this.logger.info(
                'Editor: Model reference was not updated. It is either unchanged or undefined.'
            );
        }
    }
    ...
    async checkCodeResourceUpdates(codeResources) {
        ...
        if (
            codeResources?.original !== undefined &&
    ...
    async updateCodeResources(codeResources) {
        const { updateModified, updateOriginal } =
            await this.checkCodeResourceUpdates(codeResources);
        ...
        if (this.useDiffEditor) {
            if (updateModified || updateOriginal) {
                ...
                this.logger.info(
                    'Diff Editor: Code resources were not updated. They are either unchanged or undefined.'
                );
            }
        } else if (updateModified) {
            ...
            this.logger.info(
                'Editor: Code resources were not updated. They are either unchanged or undefined.'
            );
        }
