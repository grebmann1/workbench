...
export async function initializeVscodeApi(apiConfig, startInstructions) {
    ...
    if (
        !vscodeBundle.monacoLanguageClient ||
        !vscodeBundle.monacoLanguageClient.MonacoVscodeApiWrapper
    ) {
        throw new Error(
            'MonacoVscodeApiWrapper is not available in vscodeBundle. Make sure the vscode bundle includes monacoLanguageClient.'
        );
    }
...
export function buildDefaultVscodeApiConfig(customConfig, logLevel = LogLevel.Off) {
    ...
    if (customConfig) {
        return {
            ...
            monacoWorkerFactory:
                customConfig.monacoWorkerFactory ?? defaultConfig.monacoWorkerFactory,
