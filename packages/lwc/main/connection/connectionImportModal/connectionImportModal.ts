...
export default class ConnectionImportModal extends LightningModal {
    @api newAlias: string | undefined;
    @api oldAlias: string | undefined;
    @api username: string | undefined;
    @api existingConnections: any[] = [];

    originalSize = 0;

    @track newConnections: any[] = [];

    @track list_new: string[] = [];

    isLoading = false;

    /** Methods  **/

    extractConfig = (value: string) => {
    ...
    importAndConnect = async (settings: any): Promise<ConnectorLike> => {
