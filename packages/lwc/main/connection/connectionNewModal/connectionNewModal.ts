...
export default class ConnectionNewModal extends LightningModal {
    ...
    @api customDomain: string | undefined;
    ...
    applyOrgFarmDefaultsIfNeeded() {
        ...
        // Default category to OrgFarm only when no category is selected yet.
        // Do not override if the user has explicitly chosen another category (e.g. Default or custom).
        if (!categoryId) {
            this.selectedCategory = [ORGFARM_CATEGORY];
            this.syncLookupSelection();
        } else if (isOrgFarmString(categoryId)) {
            // Normalize case/format if category is already some variant of OrgFarm.
            this.selectedCategory = [ORGFARM_CATEGORY];
            this.syncLookupSelection();
        }
