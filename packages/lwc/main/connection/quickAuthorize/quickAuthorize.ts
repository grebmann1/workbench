...
export default class QuickAuthorize extends ToolkitElement {
    ...
    @track connections: any[];
    @track exclusionList: string[] = [];
    ...
    setExclusionList = async (exclusionList: string[]) => {
    ...
    getBaseHost = (hostname: string) => {
        return hostname.split('.')[0];
    };

    isSandbox = (hostname: string) => {
        return hostname.endsWith(SALESFORCE_SANDBOX_HOST);
    };

    isStandardOrg = (hostname: string) => {
        return SALESFORCE_HOST.some(pattern => hostname.endsWith(pattern));
    };

    isInternalDevOrg = (host: string, port: string) => {
