...
export async function connect(
    { alias, loginUrl, username }: { alias?: string; loginUrl?: string; username?: string },
    settings: { bypass?: boolean; saveFullConfiguration?: boolean; persist?: boolean } = {}
): Promise<ConnectorLike> {
    const { bypass = false, saveFullConfiguration = false, persist = true } = settings;
    const effectiveAlias = alias || `direct-session-${Date.now()}`;

    const platform = getCurrentPlatform();

    // Check for existing configuration first
    const configuration = persist && effectiveAlias ? await getConfiguration(effectiveAlias) : null;
    if (configuration && configuration.refreshToken && !bypass) {
        ...
        if (persist) {
            await saveConfiguration(effectiveAlias, connector.configuration);
        }
    ...
    if (platform === PLATFORM.CHROME) {
        ...
        return new Promise((resolve, reject) => {
            ...
            chrome.runtime.sendMessage(
                ...
                async response => {
                    const { code, error, errorDescription } = response || {};
                    if (error) {
                        return reject(
                            new Error(
                                errorDescription
                        ...
                        return reject(
                            new Error(
                                response?.responseUrl
                                    ? 'OAuth flow failed: no authorization code returned'
                                    : 'OAuth flow canceled'
                            )
                        );
                    }
