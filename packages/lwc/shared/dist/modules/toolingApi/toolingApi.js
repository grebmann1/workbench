...
export function createToolingClient(options = {}) {
    const { instanceUrl, accessToken, apiVersion, proxyUrl } = options;
    const jsforceConnection = options.connection || options.connector?.conn || null;
    if (jsforceConnection) {
        ...
        const resolveUpstreamPath = (urlOrPath) => {
        ...
        return {
            ...
            proxyUrl: null, // widen literal type for interface compatibility
            requestJson,
            requestText,
            toolingQueryAll,
            async ping() {
            ...
            async listLwcBundles() {
                return await toolingQueryAll('SELECT Id, DeveloperName, NamespacePrefix FROM LightningComponentBundle ORDER BY DeveloperName');
            },
            async listLwcResources(bundleId) {
                return await toolingQueryAll(`SELECT Id, FilePath, Format FROM LightningComponentResource WHERE LightningComponentBundleId='${bundleId}' ORDER BY FilePath`);
            },
            async getLwcResourceSource(id) {
                const rows = await toolingQueryAll(`SELECT Id, FilePath, Format, Source FROM LightningComponentResource WHERE Id='${id}'`);
                return rows?.[0] || null;
            },
            async listAuraBundles() {
                return await toolingQueryAll('SELECT Id, DeveloperName, NamespacePrefix FROM AuraDefinitionBundle ORDER BY DeveloperName');
            },
            async listAuraDefinitions(bundleId) {
                return await toolingQueryAll(`SELECT Id, DefType, Format FROM AuraDefinition WHERE AuraDefinitionBundleId='${bundleId}' ORDER BY DefType`);
            },
            async getAuraDefinitionSource(id) {
                const rows = await toolingQueryAll(`SELECT Id, DefType, Format, Source FROM AuraDefinition WHERE Id='${id}'`);
                return rows?.[0] || null;
            },
        };
    }
    ...
    async function requestJson(urlOrPath, { method = 'GET', body, headers, signal } = {}) {
        ...
        try {
            ...
            const json = text
                ...
                : null;
            if (!res.ok) {
                throw new SalesforceApiError(formatSfError(res.status, json ?? text), res.status, json ?? text);
            }
            return json;
        }
        catch (err) {
            if (isLikelyCorsOrNetworkError(err)) {
                if (normalizedProxyUrl) {
                    throw new Error(`Unable to reach local proxy at ${normalizedProxyUrl}. ` +
                        'Start it (npm run sf:proxy) and retry.');
                }
                throw new Error('Network/CORS error calling Salesforce. ' +
