...
const setLayoutAssignments = async (conn, permissionSets, { permissionSetProfileMapping, layouts, }) => {
    ...
    records.forEach(record => {
        if (layouts[record.LayoutId]) {
            ...
            permissionSets[permissionSetProfileMapping[record.Profile.Id]].layoutAssigns.push(new LayoutAssignment(record.LayoutId, layouts[record.LayoutId].objectName, record.RecordTypeId));
        }
    });
};
...
function chunkPromises(arr, size, method) {
    ...
    for (const chunk of chunks) {
        collector = collector.then(results => Promise.all(chunk.map(params => method(params))).then(subResults => results.concat(subResults)));
    }
    return collector;
}
