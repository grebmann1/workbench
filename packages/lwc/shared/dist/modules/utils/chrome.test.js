...
assert(sessionFirstUrl.startsWith('https://sf-toolkit.example/views/vscode.html?sessionId='), 'session bootstrap should be serialized ahead of alias fallback metadata');
const sessionFirstSeed = parseVscodeBootstrapSeed(new URL(sessionFirstUrl).search);
assert(sessionFirstSeed.sessionId === 'session-token', 'session bootstrap should preserve the session id');
assert(sessionFirstSeed.serverUrl === 'https://example.my.salesforce.com', 'session bootstrap should preserve the server URL');
assert(sessionFirstSeed.alias === 'stale@example.com', 'session bootstrap should still carry alias metadata for fallback enrichment and alias-backed reconnect flows');
...
assert(sessionOnlySeed.sessionId === 'token-only' &&
    sessionOnlySeed.serverUrl === 'https://session-only.my.salesforce.com', 'session-only input should preserve the live bootstrap pair');
...
assert(aliasOnlySeed.alias === 'alias-only-org' &&
    aliasOnlySeed.sessionId === null &&
    aliasOnlySeed.serverUrl === null, 'alias-only input should stay alias-backed when no live session exists');
...
assert(blankAliasSessionSeed.sessionId === 'token-with-blank-alias' &&
    blankAliasSessionSeed.alias === null, 'blank alias input should normalize away while keeping the session bootstrap');
assert(buildVscodeEditorUrl({
    baseUrl: '/views/vscode.html',
    baseOrigin: 'https://sf-toolkit.example',
    seed: {
        sessionId: 'missing-server-url',
    },
}) === null, 'partial session bootstrap should not produce a malformed launch URL');
