...
class CacheManager {
    constructor() {
        this.isChrome = isChromeExtension();
    ...
    getConfigKeyMap() {
        if (!this.configKeyMap) {
            this.configKeyMap = {};
            const entries = Object.entries(CACHE_CONFIG);
...
export const CACHE_CONFIG = {
    ...
    // Google Auth Session
    GOOGLE_SESSION: new CONFIG_OBJECT('google_session', null),
    // AI Settings
    ...
    EINSTEIN_AGENT_CONVERSATION_REASONING: new CONFIG_OBJECT('einstein_agent_conversation_reasoning', null),
    ...
    METADATA_STORAGE_BACKGROUND_SYNC_ENABLED: new CONFIG_OBJECT('metadata_storage_background_sync_enabled', false),
...
function toConfigValueRecord(config) {
...
export function getDefaultLlmProviderConfigMap() {
    return createDefaultProviderConfigMap();
}
export function getLegacyLlmProviderConfigMap(config = {}) {
...
export function resolveLlmProviderConfigMap(config = {}) {
    const safeConfig = toConfigValueRecord(config);
...
export function buildProviderConfigCacheRecord(providerConfigs) {
...
export async function loadLlmProviderConfigMapFromCache() {
    const config = await cacheManager.loadConfig(getLlmProviderConfigCacheKeys());
    return resolveLlmProviderConfigMap(config);
}
export async function saveLlmProviderConfigMapToCache(providerConfigs) {
    await cacheManager.saveConfig(buildProviderConfigCacheRecord(providerConfigs));
}
export async function saveSingleLlmProviderConfigToCache(provider, config) {
...
export function getAiProviderFromConfig(config = {}) {
...
export const saveSyncedSettingsInitializedToCache = async (value = true) => {
    return await cacheManager.saveGeneralData(CACHE_CONFIG.CHROME_SYNC_SETTINGS_INITIALIZED_STORAGE_KEY.key, value, cacheManager.settingsStore);
};
