...
function allEls(doc, localName) {
    ...
    if (list2 && typeof list2.length === 'number')
...
export function createMetadataApiClient(options = {}) {
    ...
    const effectiveInstanceUrl = normalizeInstanceUrl(normalizedInstanceUrl || jsforceConnection?.instanceUrl);
    const effectiveApiVersion = normalizeApiVersion(normalizedApiVersion || jsforceConnection?.version);
    ...
    return {
        ...
        async listMetadata({ queries, asOfVersion = effectiveApiVersion } = {}) {
            ...
            if (!queriesXml)
                return [];
            const doc = await requestSoap(`<met:listMetadata>` +
                `${queriesXml}` +
        ...
        async retrieve({ typesMap, apiVersion = effectiveApiVersion } = {}) {
            ...
            const doc = await requestSoap(`<met:retrieve>` +
                `<met:retrieveRequest>` +
        ...
        async checkRetrieveStatus(id, { includeZip = true } = {}) {
            ...
            const doc = await requestSoap(`<met:checkRetrieveStatus>` +
        ...
        async deploy(zipBytes, { checkOnly = false, testLevel = 'NoTestRun' } = {}) {
            ...
            const doc = await requestSoap(`<met:deploy>` +
        ...
        async checkDeployStatus(id, { includeDetails = true } = {}) {
            ...
            const doc = await requestSoap(`<met:checkDeployStatus>` +
                `<met:asyncProcessId>${escapeXml(id)}</met:asyncProcessId>` +
