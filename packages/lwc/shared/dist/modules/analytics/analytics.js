...
async function getOrCreateClientId() {
    ...
    let clientId = configMap
        ? configMap[CACHE_ANALYTICS_CONFIG.CLIENT_STORAGE_KEY.key]
        : null;
    if (!clientId) {
        clientId =
...
async function getOrCreateSessionId() {
    ...
    const cached = configMap
        ? configMap[CACHE_ANALYTICS_CONFIG.SESSION_STORAGE_KEY.key]
        : null;
...
class AnalyticsTracker {
    configure({ measurementId, apiSecret } = {}) {
        if (!isUndefinedOrNull(measurementId))
            MEASUREMENT_ID = measurementId;
        if (!isUndefinedOrNull(apiSecret))
