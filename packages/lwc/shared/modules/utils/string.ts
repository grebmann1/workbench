...
export function lowerCaseKey(key: string | null | undefined): string | null {
...
export function stripNamespace(
    ...
    if (Array.isArray(value)) {
        return value.map(item => stripNamespace(item) as string);
    }
    const str = String(value);
    return str.replace(/^[a-zA-Z0-9]+__/, '');
}

export const splitTextByTimestamp = (text: string): string[] => {
