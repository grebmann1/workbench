...
export default class Menu extends ToolkitElement {
    ...
    handleSelect = e => {
        ...
        const external = this.externalLinks.find(o => o.name === name);
        if (external?.url) {
    ...
    generateFilter = (name, hideMenuLabel = false) => {
        ...
        filtered = filtered.filter(x => x.isMenuVisible !== false);
        if (!isElectronApp()) {
    ...
    getFilteredApplicationItemsByGroup = groupKey => {
        if (isEmpty(groupKey)) return [];
        let filtered = this.items.filter(x => x.menuGroup === groupKey);
        filtered = this.formatMenuItems(filtered);
        filtered = filtered.filter(x => x.isMenuVisible !== false);
        if (!isElectronApp()) {
            filtered = filtered.filter(x => !x.isElectronOnly);
        ...
        filtered = filtered.sort((a, b) => (a.menuOrder || 0) - (b.menuOrder || 0));
        return this.filterBySearch(filtered, this.filterText);
    };
    ...
    get others() {
        return [
            {
                name: 'extra_howTo',
                url: 'https://workbench-salesforce.com/install',
    ...
    get externalLinks() {
        return [...(this.vscodeEditorLink ? [this.vscodeEditorLink] : []), ...this.others];
    }
    ...
    get sections() {
        ...
        const othersItems = [
            ...this.filteredVscodeEditorLinks.map(toOtherItem),
