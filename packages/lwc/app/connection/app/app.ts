...
export default class App extends ToolkitElement {
    ...
    @api
    directConnectClick = async () => {
        await runQuickConnect({
            setLoading: message => this.setLoading(message),
            resetLoading: () => this.resetLoading(),
            onSuccess: (connector: ConnectorLike) => {
                this.dispatchEvent(
    ...
    login = async row => {
        if (isElectronApp() && row.credentialType === OAUTH_TYPES.OAUTH) {
            ...
            try {
                const strategy = credentialStrategies[credentialType || OAUTH_TYPES.OAUTH];
                if (!strategy)
                    throw new Error(`No strategy for credential type: ${credentialType}`);

                const connector: ConnectorLike = await strategy.connect(configuration);
        ...
        if (isElectronApp()) {
            ...
            try {
                ...
                if (isEmpty(redirectUrl)) {
                    ...
                    const connector: ConnectorLike = await strategy.directConnect(configuration);
    ...
    openToolkit = async (row, redirect) => {
        ...
        try {
            ...
            const connector: ConnectorLike = await strategy.connect({
                ...settings,
                alias,
                disableEvent: true,
            });
