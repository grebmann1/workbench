...
import { isIframeJsforceBridgeEnabled } from 'vscode/bridge/bootstrapIframeJsforceBridge'
import { connectIframeJsforceBridgeClient } from 'vscode/bridge/iframeJsforceBridgeClient'
import type { IframeJsforceBridgeHostEvent } from 'vscode/bridge/iframeJsforceBridgeContract'
import { isIframeAiBridgeEnabled } from 'vscode/bridge/bootstrapIframeAiBridge'
import { connectIframeAiBridgeClient } from 'vscode/bridge/iframeAiBridgeClient'
import type { IframeAiBridgeClient } from 'vscode/bridge/iframeAiBridgeClient'
import type { IframeAiBridgeConfigData } from 'vscode/bridge/iframeAiBridgeContract'
...
let workbenchAiBridgeClient: IframeAiBridgeClient | null = null
let workbenchAiBridgeClientPromise: Promise<IframeAiBridgeClient | null> | null = null
let workbenchAiBridgeConfig: IframeAiBridgeConfigData | null = null
const bridgeConfigListeners = new Set<(config: IframeAiBridgeConfigData) => void>()

export function getWorkbenchAiBridgeClient(): IframeAiBridgeClient | null {
  return workbenchAiBridgeClient
}

export function getWorkbenchAiBridgeConfig(): IframeAiBridgeConfigData | null {
  return workbenchAiBridgeConfig
}

export function onWorkbenchAiBridgeConfigLoaded(
  listener: (config: IframeAiBridgeConfigData) => void
): { dispose: () => void } {
  bridgeConfigListeners.add(listener)
  return { dispose: () => { bridgeConfigListeners.delete(listener) } }
}

async function resolveWorkbenchAiBridgeClient(): Promise<IframeAiBridgeClient | null> {
  if (workbenchAiBridgeClient) {
    return workbenchAiBridgeClient
  }
  if (workbenchAiBridgeClientPromise) {
    return workbenchAiBridgeClientPromise
  }
  if (!isIframeAiBridgeEnabled()) {
    return null
  }
  workbenchAiBridgeClientPromise = connectIframeAiBridgeClient()
    .then(async client => {
      workbenchAiBridgeClient = client
      workbenchAiBridgeClientPromise = null
      // Fetch and broadcast available models once the bridge is ready.
      try {
        const config = await client.fetchConfig()
        if (config) {
          workbenchAiBridgeConfig = config
          for (const listener of Array.from(bridgeConfigListeners)) {
            try { listener(config) } catch { /* ignore */ }
          }
        }
      } catch {
        // Non-fatal: model list will fall back to defaults.
      }
      return client
    })
    .catch(error => {
      console.warn('[sfWorkbench] AI bridge unavailable.', error)
      workbenchAiBridgeClientPromise = null
      return null
    })
  return workbenchAiBridgeClientPromise
...
export async function createBridgeConnectionContext({
  ...
  // Kick off AI bridge initialization in parallel with jsforce bridge
  void resolveWorkbenchAiBridgeClient()
