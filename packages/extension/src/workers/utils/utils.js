...
let contentScriptPatternsCache = null;
let contentScriptPatternsCachePromise = null;
...
export const ignoreRuntimeError = () => {
...
export const compareMajorMinor = (versionA, versionB) => {
    const [aMajor, aMinor] = versionA.split('.').map(Number);
    const [bMajor, bMinor] = versionB.split('.').map(Number);
    return aMajor !== bMajor || aMinor !== bMinor;
};

export function parsePatterns(patternString, fallback) {
    const arr = patternString ? patternString.split('\n') : fallback;
...
export async function refreshContentScriptPatternsCache(
    ...
    contentScriptPatternsCache = { includePatterns, excludePatterns };
    return contentScriptPatternsCache;
}

export async function getContentScriptPatterns(defaultIncludePatterns, defaultExcludePatterns) {
    if (contentScriptPatternsCache) return contentScriptPatternsCache;
    if (contentScriptPatternsCachePromise) return contentScriptPatternsCachePromise;

    contentScriptPatternsCachePromise = refreshContentScriptPatternsCache(
        defaultIncludePatterns,
        defaultExcludePatterns
    ).finally(() => {
        contentScriptPatternsCachePromise = null;
    });

    return contentScriptPatternsCachePromise;
}

export function normalizeUrlForPatternMatch(rawUrl) {
    try {
